...
//! on display AND while editing (the secret is never echoed). Only the top-level
//! `provider` / gateway-port / control-port keys are panel-owned (they are
//! generic daemon config, not provider specifics).
...
/// One editable row in the config form. Top-level rows (`Provider`, gateway /
/// control port) are panel-owned daemon config; [`Field`](Row::Field) rows are
/// built one-per-descriptor-entry from the selected provider's
/// `required_fields()` — the panel never names a concrete provider field itself.
...
/// unrelated keys). `provider` + ports are generic daemon config; `fields` are
/// the selected provider's opaque field map (`key` → value), keyed by
...
impl ConfigPanel {
    ...
    /// and reloads its descriptor-driven field rows instead of opening a buffer.
...
pub fn build_rows(provider: &str) -> Vec<Row> {
    let mut rows = vec![Row::Provider, Row::GatewayPort, Row::ControlPort];
    let registry = lucarne_remote::builtin();
...
/// key/section, and return the re-serialized YAML. Only `remote.provider`,
/// `remote.gateway_addr`, `remote.control_addr`, and the selected provider's
/// `remote.providers.<id>.*` field map are touched; empty field values are
/// removed so a blanked secret never lingers. An empty/blank `raw` starts from an
/// empty mapping (so a missing file is created with only the remote section).
...
mod tests {
    ...
    fn rows_built_from_provider_required_fields() {
        ...
        // First three are the panel-owned daemon-config rows.
        assert_eq!(rows[0], Row::Provider);
        assert_eq!(rows[1], Row::GatewayPort);
        assert_eq!(rows[2], Row::ControlPort);
        ...
        let field_rows: Vec<&Row> = rows.iter().skip(3).collect();
    ...
    fn secret_field_value_is_masked_never_echoed() {
        ...
        assert_eq!(mask_value("https://t.example.com", false), "https://t.example.com");
...
"#;
        ...
        assert_eq!(
            value
                .pointer("/remote/auth_token")
                .and_then(|v| v.as_str()),
            Some("keep-this-token")
        );
        assert_eq!(
    ...
    fn merge_round_trip_writes_to_temp_file_via_backup() {
        ...
        let backups: Vec<String> = std::fs::read_dir(temp.path())
            ...
            .collect();
        assert_eq!(backups.len(), 1, "exactly one backup of the previous config");
    ...
    fn empty_field_is_removed_from_provider_map_on_merge() {
        ...
        let edits = ConfigEdits {
            ...
            gateway_port: 7800,
            control_port: 7801,
            fields: BTreeMap::new(), // token edited to empty → dropped
        };
    ...
    fn cycle_provider_rebuilds_descriptor_rows() {
        ...
        panel.list.select(Some(0)); // Provider row
        ...
        assert_eq!(panel.rows[0], Row::Provider);
    ...
    fn inline_edit_commits_non_secret_field_and_parses_port() {
        let mut panel = ConfigPanel::new();
        // Select the gateway-port row (index 1) and edit it.
        panel.list.select(Some(1));
        ...
        panel.handle_key(KeyCode::Enter);
        assert_eq!(panel.edits.gateway_port, 9100);
        assert!(panel.status.as_deref().unwrap_or("").contains("invalid port"));
    }
